    ...
    def handle_snake_collisions(self):
        ...
        for snake_a in self.snakes:
            if snake_a.dead:
                continue
            head_position = (
                self.wraparound(snake_a.body[-1][0]),
                self.wraparound(snake_a.body[-1][1]),
            )
            for snake_b in self.snakes:
                if snake_a == snake_b or snake_b.dead:
                    continue
                snake_b_positions = [
                    (self.wraparound(part[0]), self.wraparound(part[1]))
                    for part in snake_b.body
                ]
